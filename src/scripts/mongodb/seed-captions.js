...
// From server/src/scripts/mongodb -> up to server -> data/captions/captions.json
const CAPTIONS_PATH = path.resolve(__dirname, "../../../data/captions/captions.json");
...
const seedCaptions = async () => {
    ...
    try {
        const raw = await fs.readFile(CAPTIONS_PATH, "utf-8");
        const captions = JSON.parse(raw);

        const limitedCaptions = Array.isArray(captions)
            ? captions.slice(0, limit ?? captions.length)
            : [];

        if (!limitedCaptions.length) {
            console.warn("No caption entries to seed");
            return;
        }

        const candidates = [];
        for (const entry of limitedCaptions) {
