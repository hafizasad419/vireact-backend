...
// From server/src/scripts/mongodb -> up to server -> data/advanced-analytics/advanced-analytics.json
const ANALYTICS_PATH = path.resolve(__dirname, "../../../data/advanced-analytics/advanced-analytics.json");
...
const seedAdvancedAnalytics = async () => {
    if (!DB_URL) {
    ...
    try {
        ...
        const limitedAnalytics = Array.isArray(analytics)
            ? analytics.slice(0, limit ?? analytics.length)
            : [];

        if (!limitedAnalytics.length) {
            console.warn("No advanced analytics entries to seed");
            return;
        }

        const candidates = [];
        for (const entry of limitedAnalytics) {
...
seedAdvancedAnalytics().catch((err) => {
