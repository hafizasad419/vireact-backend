...
// From server/src/scripts/mongodb -> up to server -> data/views-predictor/views-predictor.json
const VIEWS_PREDICTOR_PATH = path.resolve(__dirname, "../../../data/views-predictor/views-predictor.json");
...
const seedViewsPredictor = async () => {
    if (!DB_URL) {
    ...
    try {
        const raw = await fs.readFile(VIEWS_PREDICTOR_PATH, "utf-8");
        const viewsPredictor = JSON.parse(raw);

        const limitedViewsPredictor = Array.isArray(viewsPredictor)
            ? viewsPredictor.slice(0, limit ?? viewsPredictor.length)
            : [];

        if (!limitedViewsPredictor.length) {
            console.warn("No views predictor entries to seed");
            return;
        }

        const candidates = [];
        for (const entry of limitedViewsPredictor) {
