...
// From server/src/scripts/mongodb -> up to server -> data/hooks/hook.json
const HOOKS_PATH = path.resolve(__dirname, "../../../data/hooks/hook.json");
...
const buildDocsFromEntry = (entry) => {
    const score = coerceScore(entry["Score (0.0-1)"]);
    const layers = [
        { key: "raw", layer: "raw" },
...
const seedHooks = async () => {
    ...
    try {
        const raw = await fs.readFile(HOOKS_PATH, "utf-8");
        const hooks = JSON.parse(raw);

        const limitedHooks = Array.isArray(hooks)
            ? hooks.slice(0, limit ?? hooks.length)
            : [];

        if (!limitedHooks.length) {
            console.warn("No hook entries to seed");
            return;
        }

        // Optional: clear existing hook docs to avoid duplicates
        await KnowledgeBase.deleteMany({ "metadata.topic": "hook" });

        const candidates = [];
        for (const entry of limitedHooks) {
