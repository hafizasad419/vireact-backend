...
// From server/src/scripts/mongodb -> up to server -> data/audio/audio.json
const AUDIO_PATH = path.resolve(__dirname, "../../../data/audio/audio.json");
...
const seedAudio = async () => {
    ...
    try {
        const raw = await fs.readFile(AUDIO_PATH, "utf-8");
        const audio = JSON.parse(raw);

        const limitedAudio = Array.isArray(audio)
            ? audio.slice(0, limit ?? audio.length)
            : [];

        if (!limitedAudio.length) {
            console.warn("No audio entries to seed");
            return;
        }

        const candidates = [];
        for (const entry of limitedAudio) {
